...
void main()
{
	...
	vec3 viewPos = vec3(0.0, 0.0, -1.0); //not the most correct approch
	vec3 viewDir = normalize(viewPos - fragPos);
	vec3 reflectvec = reflect(-lightdir,normal);
	float specFac = max(dot(reflectvec,viewDir),0.0);
	spec = pow(specFac,phongExp);
	float diffuseAlign = max(dot(lightdir,normal),0.0);
	diff = 0.4*diffuseAlign;
